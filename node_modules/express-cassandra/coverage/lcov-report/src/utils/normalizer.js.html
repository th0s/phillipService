...
254
255
256</td><td class="line-coverage quiet"><span class="cline-any cline-yes">1x</span>
...
&nbsp;
  normalize_query_option(options) {
    ...
    if (options.consistency) <span class="cstat-no" title="statement not covered" >queryOptions.consistency = options.consistency;</span>
    if (options.fetchSize) queryOptions.fetchSize = options.fetchSize;
    if (options.autoPage) <span class="cstat-no" title="statement not covered" >queryOptions.autoPage = options.autoPage;</span>
    if (options.hints) <span class="cstat-no" title="statement not covered" >queryOptions.hints = options.hints;</span>
    if (options.pageState) <span class="cstat-no" title="statement not covered" >queryOptions.pageState = options.pageState;</span>
    if (options.retry) <span class="cstat-no" title="statement not covered" >queryOptions.retry = options.retry;</span>
    if (options.serialConsistency) <span class="cstat-no" title="statement not covered" >queryOptions.serialConsistency = options.serialConsistency;</span>
    if (options.customPayload) <span class="cstat-no" title="statement not covered" >queryOptions.customPayload = options.customPayload;</span>
    if (options.isIdempotent) <span class="cstat-no" title="statement not covered" >queryOptions.isIdempotent = options.isIdempotent;</span>
    if (options.readTimeout) <span class="cstat-no" title="statement not covered" >queryOptions.readTimeout = options.readTimeout;</span>
    if (options.retry) <span class="cstat-no" title="statement not covered" >queryOptions.retry = options.retry;</span>
    if (options.retryOnTimeout) <span class="cstat-no" title="statement not covered" >queryOptions.retryOnTimeout = options.retryOnTimeout;</span>
    if (options.routingIndexes) <span class="cstat-no" title="statement not covered" >queryOptions.routingIndexes = options.routingIndexes;</span>
    if (options.routingKey) <span class="cstat-no" title="statement not covered" >queryOptions.routingKey = options.routingKey;</span>
    if (options.routingNames) <span class="cstat-no" title="statement not covered" >queryOptions.routingNames = options.routingNames;</span>
    if (options.timestamp) <span class="cstat-no" title="statement not covered" >queryOptions.timestamp = options.timestamp;</span>
    return queryOptions;
  },
&nbsp;
...
&nbsp;
    if (outputSchema.custom_indexes) {
      const customArraySort = (a, b) =&gt; {
        if (a.on &gt; b.on) <span class="cstat-no" title="statement not covered" >return 1;</span>
        if (a.on &lt; b.on) return -1;
&nbsp;
        if (a.using &gt; b.using) <span class="cstat-no" title="statement not covered" >return 1;</span>
        if (a.using &lt; b.using) <span class="cstat-no" title="statement not covered" >return -1;</span>
&nbsp;
        if (a.options &gt; b.options) <span class="cstat-no" title="statement not covered" >return 1;</span>
        if (a.options &lt; b.options) <span class="cstat-no" title="statement not covered" >return -1;</span>
&nbsp;
        return 0;
      };
...
<span class="cstat-no" title="statement not covered" >    const dependentViews = [];</span>
    Object.keys(normalizedDBSchema.materialized_views).forEach(<span class="fstat-no" title="function not covered" >(dbViewName) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >      if (normalizedDBSchema.materialized_views[dbViewName].select.includes(fieldName)) {</span>
    ...
    dependentViews.forEach(<span class="fstat-no" title="function not covered" >(viewName) =&gt; {</span>
